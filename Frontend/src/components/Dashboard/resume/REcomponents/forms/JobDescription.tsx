...
export const JobAnalysisDisplay: React.FC<{
  ...
  const checkKeywordsInResume = (keywords: string[]): KeywordItem[] => {
    if (!resumeInfo)
      return keywords.map((k: string) => ({ keyword: k, isFound: false }));

    const resumeText = [
      resumeInfo.summary || "",
      resumeInfo.jobTitle || "",
      // Experience section (note: it's "Experience" with capital E in your types)
      ...
      ...(resumeInfo.education || []).map(
        (edu: EducationItem) =>
          `${edu.degree || ""} ${edu.major || ""} ${edu.universityName || ""} ${
            edu.description || ""
          }`
      ),
      ...
      ...(resumeInfo.skills?.other || []).map((skill) => skill.name || ""),
      // Projects section
      ...
      ...(resumeInfo.certifications || []).map((cert) => cert.title || ""),
    ]
      .join(" ")
      .toLowerCase();
