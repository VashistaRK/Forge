...
const Summary: React.FC<SummaryProps> = ({ enableNext }) => {
  ...
  const saveJobTitle = async (newTitle: string): Promise<void> => {
    ...
    try {
      ...
      // Update state
      setResumeInfo((prev) => (prev ? { ...prev, jobTitle: newTitle } : null));
  ...
  const handleInputChange = (e: ChangeEvent<HTMLTextAreaElement>) => {
    ...
    setResumeInfo((prev) => (prev ? { ...prev, summary: newSummary } : null));
  };
  ...
  const handleSuggestionClick = (item: SummarySuggestion) => {
    ...
    setResumeInfo((prev) => (prev ? { ...prev, summary: selected } : null));
